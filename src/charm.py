...
class TLSRequirerCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        if common_name and not self.unit.is_leader():
            logger.warning(
                "Will not request certificate for non leader unit when common name is set."
            )
            return
    ...
    def _collect_status_unit_mode(self) -> StatusBase:
        ...
        if not self._config_is_valid():
            return BlockedStatus("Invalid unit configuration")
    ...
    def _configure(self, event: EventBase) -> None:
        ...
        if not self._mode_config_is_valid():
            ...
            return
        if not self._config_is_valid():
            logger.error("Invalid unit configuration")
            return
        if not self._certificates_relation_created:
            return
        if mode == Mode.APP and not self.unit.is_leader():
    ...
    def _configure_unit_mode(self):
        ...
        for certificate_request in self._get_certificate_requests():
            if not self._certificate_is_stored(certificate_request=certificate_request):
                self._store_unit_certificate(certificate_request=certificate_request)

    def _configure_app_mode(self):
        """Manage certificate lifecycle when they are managed per application."""
        for certificate_request in self._get_certificate_requests():
            if not self._certificate_is_stored(certificate_request=certificate_request):
                self._store_app_certificate(certificate_request=certificate_request)
            ...
            if self._certificate_secret_exists(certificate_request=certificate_request):
                try:
                    certificate_secret = self.model.get_secret(
                        label=self._get_certificate_secret_label(certificate_request)
                    )
    ...
    def _config_is_valid(self) -> bool:
        """Return whether unit configuration is valid.

        Rules:
        - If common_name is set, num_certificates must be 1.
        """
        if self._get_config_common_name() and self._get_config_num_certificates() > 1:
            return False
        return True
    ...
    def _store_unit_certificate(self, certificate_request: CertificateRequest) -> None:
    ...
    def _store_app_certificate(self, certificate_request: CertificateRequest) -> None:
    ...
    def _on_get_certificate_action(self, event: ActionEvent) -> None:
        ...
        if mode == Mode.UNIT:
            if not self._config_is_valid():
                event.fail("Invalid unit configuration")
                return
            certificates = []
            for certificate_request in self._get_certificate_requests():
